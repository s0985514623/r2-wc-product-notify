...
class R2_cron
{
	...
	// {
	// 	wp_mail('s0985514623@gmail.com', 'before__construct', 'message');
	// 	// add_filter('cron_schedules', array($this, 'r2_add_cron_interval'));
	// 	// add_action('wp', array($this, 'custom_daily_cron_schedule'));
	// 	// add_action('bl_order_date_cron_Hook', array($this, 'bl_order_date_cron_Exec'));
	// 	wp_mail('s0985514623@gmail.com', 'after__construct', 'message');
	// }
	public function init()
	{
		add_filter('cron_schedules', array($this, 'r2_add_cron_interval'));
		// add_action('init', array($this, 'custom_cron_schedule'));
	...
	public function set_cron_schedule($to, $subject, $message)
	{
		if (!wp_next_scheduled('bl_order_date_cron_Hook', array($to, $subject, $message))) {
			wp_schedule_event(time(), 'every_minute', 'bl_order_date_cron_Hook', array($to, $subject, $message));
		}
